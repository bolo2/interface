    ...
    return (
        <div id={id} className="rounded bg-dark-800 p-5">
            <div className="flex flex-col space-y-3 sm:space-y-0 sm:flex-row justify-between">
                ...
                <div className="flex items-center rounded bg-white font-bold text-xl space-x-3 p-3 w-full sm:w-3/5">
                    <PercentInput
                        className="token-amount-input text-gray-800"
                        value={value}
                        onUserInput={val => {
                            onUserInput(val)
                        }}
                        align="right"
                    />
                    <div className="font-bold text-xl pl-2 text-gray-400">%</div>
                </div>
            </div>
        </div>
    )
